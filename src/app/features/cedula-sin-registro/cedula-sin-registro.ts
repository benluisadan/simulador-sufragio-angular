...
@Component({
  selector: 'app-cedula-sin-registro',
  standalone: true,
  imports: [NgIf, NgFor, CommonModule, FormsModule],
  templateUrl: './cedula-sin-registro.html',
  styleUrls: ['./cedula-sin-registro.scss']
})

export class CedulaSinRegistro implements OnInit {
  ...
  //Correo
  popupCorreoVisible = false;
  correoUsuario = '';
  //ubigeo
  popupUbigeoVisible = false;

  nombreUbigeo = 'LIMA / LIMA / LIMA';
  ...
  provinciaSeleccionada = '01';
  distritoSeleccionado = '01';



  async ngOnInit(): Promise<void> {
    this.procesoId = 1;
    this.ubigeoId = '140101'; // Lima/Lima/Lima
    ...
    await this.inicializarVotanteAnonimo();
  ...
  cargarDepartamentos() {
    this.ubigeoService.getDepartamentos().subscribe({
      next: (data) => {
        ...
        const lima = this.departamentos.find(d => d.id === '14');
        if (lima) {
  ...
  cargarProvincias() {
    this.ubigeoService.getProvincias(this.departamentoSeleccionado).subscribe({
      next: (data) => {
        this.provincias = data;

        // Seleccionar Lima por defecto (01)
        this.provinciaSeleccionada = this.provincias[0].id;
  ...
  cargarDistritos() {
    this.ubigeoService.getDistritos(
      ...
      next: (data) => {
        ...
        // Seleccionar primer distrito por defecto
        this.distritoSeleccionado = this.distritos[0].id;
        this.cd.detectChanges(); // ← NECESARIO
  ...
  confirmarEnvioCorreo() {
    ...
    this.cedulaService.enviarResumenVotoSinRegistro(this.votanteId, this.correoUsuario).subscribe();
    this.popupCorreoVisible = false;
    alert('Tu simulación ha sido enviada a tu correo. si no aparece en tu bandeja revisa en correos no deseados.');
  ...
  cerrarPopupUbigeo() {
    this.popupUbigeoVisible = false;
  }

  confirmarUbigeo() {
    if (this.departamentoSeleccionado.length > 2) {
      this.ubigeoId =
        this.departamentoSeleccionado.slice(0, 2) +
        this.provinciaSeleccionada +
    ...
    this.recalcularUbigeo();
    this.reiniciarTimer();

    this.cargarSenadoMultiple();
    this.cargarDiputados();

    this.nombreUbigeo = this.obtenerNombreUbigeo();
  ...
  cambiarUbigeo() {
    this.recalcularUbigeo(); // misma lógica que ya tienes
    this.reiniciarTimer();
